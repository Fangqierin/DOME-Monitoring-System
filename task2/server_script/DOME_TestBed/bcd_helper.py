...
def imshow_EFA(area_map,r=1,c=1,i=1,figsize=(5,5),cmap="viridis"):
    ...
    norm = mcolors.DivergingNorm(vmin=0, vmax = 5, vcenter=2)
...
def DrawTask(tasks, EFAM):
    ...
    codict={'BM':0,'FT':1,'FI':2, 'B':3,'FD':4}
...
def imshow_Task(area_map,r=1,c=1,i=1,cmap="viridis"):
    """
    Display with no interpolation.
    """
    # if r < 2 and c < 2 or i == 1:
    ...
    # plt.ylim(0,ly-1)
    #plt.axis('off');
    colors = [ im.cmap(im.norm(value)) for value in range(5)]
    lab=['BM', 'FT', 'FI','BM&FT','FD']
    #color['r', 'g', 'b','r','g']
    patches = [ mpatches.Patch(color=colors[i], label=lab[i] ) for i in values ]
