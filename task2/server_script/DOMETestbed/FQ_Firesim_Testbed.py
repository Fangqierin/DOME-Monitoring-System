...
def ClearBarrier(data):
    ...
    newdata.loc[0, 'geometry'] = u2.boundary
...
def WriteInput(foldername, dir, step=1, initime=(0, 0), dura=(2, 0), dis_res=5,
               pre_res=5):  # The simulation time 100 =1:00) duration<24 hours!
...
def PutSite(data, UID, Len, Width, foldername, dir, offset=20):
    if len(UID) == 1:
        ...
        # offset=20
        boundary = (
        int(min(x) + offset), int(min(y) + offset), int(min(x) + offset + Len), int(min(y) + offset + Width))
...
def AddFireGrid(foldername, dir, prefix, time, poly):  # Update the ignition file! because we add some fire!!!!
    ...
    try:
        ...
        geoms = [data.loc[i, 'geometry'] for i in range(len(data))] + [poly.loc[i, 'geometry'] for i in
                                                                       range(len(poly))]
...
def UpdateFireIgnition(foldername, dir, BunsiteBound, Grid_map, Res, time, simdur, TM):
    ...
    if len(x) == 0:
        ...
        EFA, EFAdict, bound = GetEFA(inittime, simdur, BunsiteBound, dir, foldername,
                                     Res=Res)  # Read the output, get EFA
        ...
        if len(Unvisited) == 0:
            ...
            for i in range(r):
                for j in range(w):
                    ...
                    EFACollection.update_one({'Cords': (j, i)}, {
                        '$set': {'Grid ID': int(get['Grid ID']), 'Cords': (j, i), 'EFA': int(EFA[j, i])}})
...
def output(grids):
    ...
    WPSeq = AllCompTestbed(TANum, GWP, FPnum, Drones, init, Plantime, inloc, GCloc, Missions, DecomposeSize, EFA, Res,
                           tasks, log='', seed=seed, DrawWaypoinSequence=False)
    return EFA, tasks, WPSeq


# Mission Configuration
Missions = defaultdict(dict)  # Missions: period, priority,
Missions['BM'] = [1, 1]  # Burn Site Resource Monitoring (human and equipment)
Missions['FI'] = [0.5, 1]  # Fire Intensity Inspection
Missions['FT'] = [1, 1]  # track the fire perimeter and the risky area (arrival within 10 min)
...
speeds = [3]  # Drone Speed
Plantime = 60 * 4  # Planning Time
##### Sensor Information
...
DroneNum = 1
simdur = 40
TM = TaskManager(Missions, theta=1, plantime=simdur)  # Create the task manager!
###################################
dir = 'farsite'
