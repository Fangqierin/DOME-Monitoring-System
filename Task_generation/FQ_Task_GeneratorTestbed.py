...
class TaskManager():
    ...
    def StartWithUK(self, EFAM, init=0):
        #print(f"see empty tasks  {tasks}")
        rows,columns=EFAM.shape
        self.latime=init
        self.EFA=EFAM
        for x in range(rows):
            for y in range(columns):
                ...
                # else:
                #     st='N'
                self.TG.declare(Grid(id=(x,y),state='UK', EFA=EFAM[x,y],time=init))
        self.TG.run()
        self.cur_tasks=tasks
        self.reset()
        return self.cur_tasks
...
class TaskGenerator(KnowledgeEngine):
    ...
    def FT(self,f,id,t,EFA,ct,v, plan,BM):
        if max(t,ct)>=EFA-v:
            ...
            print(f"FTTT plan {plan} {EFA} {v} {max(t,ct)} {EFA-v}")
            ...
            if EFA-v-max(t,ct)>=BM:
                ...
                if ent>=plan:
                    ...
                    tasks[id]['FT']=(ent, -1)
            print(f"FTTT plan {plan} {EFA} {v} {max(t,ct)} {ent} {EFA-v}")
