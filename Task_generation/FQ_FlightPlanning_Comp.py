    ...
    def initialTask(self):     #Here we only consider to handle started task!
        for a in self.Area_Task:
            for t in self.Area_Task[a]: 
                ...
                #print(f"whyy", t[0],offset,(t[1][0]*60, t[1][1]*60 if t[1][1]!=-1 else -1) )
        ...
        for m , dl in self.Mission_DL.items():  # Check the dl! 
            ...
            if dl<self.time:
                ...
                #     logfile.write(f"Drone {self.drone.id} Miss Deadline  {m} DL {dl} at {self.time}\n")
                                ###############################
                #print(f"Drone {self.drone.id} Miss Deadline  {m} DL {dl} at {self.time}\n")
                ...
                for grid in self.Mission_Grid[m]:
                    ...
                    #print(f"Add {len(CurArea)} mission {m} to drone {self.drone.id} at time {self.time}  \n")
                weight=1
                ...
                #print(f"Miss {jrcount} mission  {m}  weight {weight}  at time {self.time}\n")
    ...
    def DL_Release_FP(self,EndTime, FPnum, Improve): 
        ...
        while self.time<=EndTime:
            ...
            while count<len(Gridset) and NoUpDL and self.time<=EndTime:
                CurGrids=copy.copy(Gridset[count])
                CurGrids=[g for g in CurGrids if len(self.Grid_toCov[g])>0 ]
                #print(f"check CurGrids {CurGrids}")
                if len(CurGrids)>0:
            ...
            if not Improve and count==len(Gridset) and NoUpDL: 
                nwpc=self.ShortUpload.get(self.wp)[1]
                #self.waypointSeq.append(nwpc) -------Try
                #self.DrawWPsequence()
                reward,SReward,Penalty,NoUpDL=self.ClusterStateTrans(nwpc)
                Reward=Reward+reward
                P=P+Penalty
        ...
        #print(f" drone {self.drone.id} waypoints {Reward}, {P}, {len(self.waypointSeq)} {self.waypointSeq}")
        return Reward, P, time.time()-st
    def GenWaypoint_SetCoverFake(self):
        allh=[]
        for tak in list(self.drone.MissionSet):
            heights=self.drone.WPCinfo.get(tak)
            tmph=[[h, heights[h][0]] for h in heights]
            ...
            self.WPC_TakFov[tak]=tmph
        x0,y0,z=np.array(self.GCloc)
        px=[i for i in range(int(x0-self.drone.range/self.Res),int(x0+self.drone.range/self.Res)) if i>=0 and i%(2)==0]
        #######Changed for New Res
        #px=[i for i in range(x0-self.drone.range//self.Res,x0+self.drone.range//self.Res) if i>=0 and i%(2)==0]
        allz= list(range(int(min(allh)), int(max(allh)),int(self.Res*2)))+[ int(max(allh))]
        yrange=[y0-self.drone.range, y0+self.drone.range ]
        upwpc=set()
        for x in px:
            for z in allz:
                tmp=(self.drone.range)**2-z**2-((x-x0)*self.Res)**2
                if tmp>=0:
                    y1=(np.sqrt(tmp))/self.Res+y0 # Changed for rES
                    y2= y0-(np.sqrt(tmp))/self.Res 
                    upwpc.add((x,int(y1),z))
                    upwpc.add((x,int(y2),z))
        UW=list(upwpc)
            ...
            for wpc in tmph:
                ...
                for x in allx:#[min(xal)]+tmp+[max(xal)-fov]:
                    ...
                    for y in ally:
                        ...
                        ########################
        self.SetCoveWPC()
    def DL_Release_FP_TestBed(self,EndTime, FPnum, Improve): 
        #self.DrawWPCs()
        self.EndTime=EndTime
        self.waypointSeq.append(self.wp)
        ########Define a NOWPCGrid 
        ...
        #print(f"Planning time ")    
        #print(f" drone {self.drone.id} waypoints {Reward}, {P}, {len(self.waypointSeq)} {self.waypointSeq}")
        return Reward, P, time.time()-st, self.waypointSeq
IFLOG=False
...
def AllComp(TANum,GWP,FPnum,Drones,init, Plantime,inloc,GCloc, Missions,DecomposeSize,EFA, Res,tasks,log='',seed=0):
    ...
    LogReward=[]
    for drone in Drones:
        planner=FlightPlanner(drone,init=init, iniloc=inloc, GCloc=GCloc,Missions=Missions,Res=Res) # Here, GCloc and iniloc divided by Res!
        if IFLOG:
            planner.logfile=log
        if GWP==1:
...
def AllCompTestbed(TANum,GWP,FPnum,Drones,init, Plantime,inloc,GCloc, Missions,DecomposeSize,EFA, Res,tasks,log='',seed=0):
    if IFLOG:
        log.write(f"Start {TANum} {GWP} {FPnum} {seed}\n")
    AreaPara=Decomposition(DecomposeSize,Res,tasks) # Need put tasks there!!!
    ct=time.time()
    ...
    for drone in Drones:
        ...
        if IFLOG:
            planner.logfile=log
        if GWP==1:
            planner.GenWaypoint_SetCoverFake()
        if GWP==2:
            planner.GenWaypoint_Adjust()
        if GWP==3:
            planner.GenWaypoint_Regular()
        if FPnum==0:
            ...
            Reward, P, Runtim, WaypoinSeq=planner.DL_Release_FP_TestBed(Plantime, FPnum,Improve=False )
        LogTask.append(planner.logtasks)
        LogMiss.append(planner.logmiss)
        LogReward.append(planner.logReward)
        Rewardlist.append(Reward)
        Penaltylist.append(P)
        Runtimelist.append(Runtim)
        WP.append(WaypoinSeq)
        # if DrawWaypoinSequence:
        #     planner.DrawWPsequence()
    print(f"Reward {sum(Rewardlist)} Penalty {sum(Penaltylist)} ")
    return WP
    #return TaskAllTime,Rewardlist, Penaltylist, Runtimelist,LogTask,LogMiss,LogReward
