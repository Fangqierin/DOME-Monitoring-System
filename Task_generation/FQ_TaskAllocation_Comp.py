...
def ExtrmLocDroneFix(Bidders, GC, GCloc, Grids, sed, Res):
    def Distance(x,y):
        return math.sqrt((x[0]-y[0])**2+(x[1]-y[1])**2)
    DNum=len(Bidders)
    gridBM=[i for i in Grids if i[0]=='BM']
    ...
    donBM=sorted([i for i in dronestate if i[1]=='RGB']+addF, key=lambda x: x[2], reverse=True) #long --> short
    asgG=[];loc={}
    if len(donF)==0 or len(donBM)==0:########### One link sensor
        alldrone=sorted([i for i in dronestate], key=lambda x: x[2], reverse=True) #long --> short
        gridAll=[i for i in Grids]
        for i in range(len(alldrone)):
            if i==0:
                Dis=[Distance(g[-1],GCloc) for g in gridAll]
            elif len(donF)>1 and i==1:
                Dis=[-1*Distance(g[-1],GCloc) for g in gridAll]
            else:
                Dis=[sum([Distance(g[-1],j[-1]) for j in asgG]) for g in gridF]
            m_d=Dis.index(max(Dis))
            asgG.append(gridAll[m_d])
        GridsAll=sorted([(i,Distance(i[-1],GCloc )) for  i in asgG], key=lambda x:x[-1]) 
        for i in range(len(alldrone)):
            loc[alldrone[i][0]]=GridsAll[i][0]
        return loc
    ###############################################################
    asgG=[];loc={}
    asgBM=[]; asgF=[]
...
class Bidder:
    ...
    def PretreatWPC(self,Drone, Grids):
        ...
        for g in Grids:
            ...
            if FOV!=-1:
                ...
                WPCset=set()
                for a in areas:
                    #print(f"FQcheck", a,FOV)
                    FOV=self.Res # Here ----> For TestBed
                    #x,y,_=np.array(a)*2
                    #print(a[0],a[1])
    ...
    def Get_ImproveUT(self, g):
        ...
        if len(WPCs-CurWPCs)==0:  # if WPCs contains in current WPCs
            self.Tak_WSeq
            if m not in list(self.Tak_WSeq.keys()):
                self.Tak_WSeq[m]=[]
...
def Auction_Comp(which, AreaPara, Drones, Res,Missions, seed, EFAM, GCloc):
    ...
    gcloc=np.array(GCloc)//Res
    loc=ExtrmLocDroneFix(Bidders, seed, gcloc, Grids, seed, Res)
